...
В этой задаче вам нужно реализовать упрощенный аналог шаблонного класса `std::deque`. В отличие от ветора элементы в деке расположены не непрерывно, а поделены на блоки (внутри блока непрерывность сохраняется). В нашей задаче размер блока - 512 байт.
Указатели на блоки хранятся в отдельном массиве. Если в процессе вставки элемента в деку все блоки оказались заняты, то выделяется новый блок и элемент помещается в него. Таким образом, никакие ссылки/указатели на уже хранящиеся элементы деки не инвалидируются, ведь переаллокации блоков не происходит.
